(function(win, $){
	function clone(src,out){ //프로토타입의 속성들을 복제하는 clone() 함수
	...
	function Rect(){ //직사각형 객체 Rect를 만들고, 앞서 만든 clone으로 Circle의 프로토타입을 복제
	...
	BlueCircleBuilder.prototype.init = function() {
		...
		var rect = new Rect(); //파란색 원이 생길 때, 그 옆에 노란색 직사각형 생성
				rect.color("yellow");
				rect.move(40,40);
	...
	$(win.document).ready(function(){
		...
		cg.register('blue',BlueCircleBuilder)//추상화
		cg.setStage($('.advert'))
