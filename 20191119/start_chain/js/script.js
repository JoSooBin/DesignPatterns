(function(win, $){
	...
	Circle.prototype.next = function(shp){
		...
		return this.nextShape;//두번째 원이 만들어지면 첫번째 만들어진 원의 next이다. 이 후 세네번째 만들어지는것도 동일
	};

	Circle.prototype.chainDo = function(action, args, count){//컬러와 액션이 랜덤으로 들어오고 들어온 갯수만큼(맨 마지막꺼)의 색과 액션이 동일 적용
		this[action].apply(this, args);
		if(count &&this.nextShape){
			this.nextShape.chainDo(action, args, --count)//감소하면서 모두 적용
		}
	}
		...
		function init(){
			...
			function create(left, top,type){
				var circle = _sf.create(type);
					index = _aCircle.length -1;//원들 연결
				circle.move(left, top);
				circle.setID(_aCircle.length);
				_aCircle.push(circle);//배열에 담음
				
				if(index != -1) {//배열안에 원이 담기기 직전의 인덱스값 측정
					_aCircle[index].next(circle);//지금 만들어진 circle이 next이다
				}

				return shapeFacade(circle);
			}

			function chainTint(count){//랜덤하게 색상 바꾸는 중
				var index = Math.max(0, _aCircle.length -5),
					clr = '#' + Math.floor(Math.random()*100) +
